...
Electron MessagePorts should be the v1 desktop transport between the Svelte
renderer and the backend/core runtime. The Electron main process should broker
the connection, but routine application traffic should use transferred ports
rather than a growing collection of unrelated IPC handlers.

MessagePort is only the delivery mechanism. Artisan should own a transport-safe,
...
messages rather than HTTP methods and URL paths. MessagePort is the duplex
transport; the protocol router is the application boundary that decodes an
...
MessagePorts directly. For request/response interactions, the client should keep
...
V1 should use two logical ports:
...
Separating control and stream traffic prevents terminal or model output from
