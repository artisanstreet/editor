<script lang="ts" effect>
	...
	let {
		...
		disabled: boolean;
		model: ModelChoice;
		oncontext: (model: ModelChoice, option: ContextWindowChoice) => void;
		onpermission: (model: ModelChoice, option: PermissionOption) => void;
		onspeed: (model: ModelChoice, option: SpeedOption) => void;
		onthinking: (model: ModelChoice, level: ThinkingLevel) => void;
...
<div class="flex flex-col gap-1.5">
	{#if thinking.availability === "supported" && current_thinking !== undefined}
		<Select
			...
			onValueChange={(value) => onthinking(model, value as ThinkingLevel)}
			{disabled}
		>
	...
	{#if speeds.length > 1}
		<Select
			...
			onValueChange={(value) => {
				...
				if (option !== undefined) onspeed(model, option);
			}}
			{disabled}
		>
			<div class="card min-w-0 rounded-md bg-linear-to-b from-surface-225 to-surface-200 dark:from-surface-800 dark:to-surface-925">
	...
	{#if context !== undefined && current_context !== undefined}
		<Select
			...
			onValueChange={(value) => {
				...
				if (option !== undefined) oncontext(model, option);
			}}
			{disabled}
		>
			<div class="card min-w-0 rounded-md bg-linear-to-b from-surface-225 to-surface-200 dark:from-surface-800 dark:to-surface-925">
	...
	{#if permission_options.length > 1 && current_permission !== undefined}
		<Select
			...
			onValueChange={(value) => {
				...
				if (option !== undefined) onpermission(model, option);
			}}
			{disabled}
		>
			<div class="card min-w-0 rounded-md bg-linear-to-b from-surface-225 to-surface-200 dark:from-surface-800 dark:to-surface-925">
