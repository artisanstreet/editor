<script lang="ts" effect>
	...
	import ShaderDevPanel from "../shader-dev-panel.svelte";
	...
	const ClosePanel = Effect.gen(function* () {
		panel_state = "closing";
		const FinishClose = Effect.gen(function* () {
			yield* Effect.sleep("150 millis");
			panel_state = "closed";
		});
		close_fiber = yield* FinishClose.pipe(Effect.forkScoped);
	});
	...
	const HandleKeydown = (event: KeyboardEvent) =>
		Effect.gen(function* () {
			if (event.key === "Escape" && panel_state === "open") yield* ClosePanel;
		});
</script>
