...
/// Computes the JavaScript-compatible unsigned code-point FNV-1a hash of a
/// seed.
///
/// JavaScript `for...of` iterates Unicode code points, which corresponds to
/// Rust's [`str::chars`] for valid UTF-8 input. `Math.imul(... ) >>> 0` is the
/// corresponding wrapping `u32` multiplication below. This intentionally
/// does not iterate UTF-16 code units: a non-BMP character contributes its
/// single code point, exactly as `character.codePointAt(0)` does in the
/// source policy.
#[must_use]
pub fn code_point_fnv1a(seed: &str) -> u32 {
...
/// Names the JavaScript compatibility boundary explicitly.
#[must_use]
pub fn javascript_code_point_fnv1a(seed: &str) -> u32 {
    code_point_fnv1a(seed)
}
