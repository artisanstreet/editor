...
export const ForgeChildEnvironment = (
  ...
  ARTISAN_BROKER_PATH: artifact.broker_executable_path,
  ...
  ARTISAN_FORGE_MODE: input.config.mode ?? "local",
  ARTISAN_FORGE_STATE_PATH: paths.state_path,
  ARTISAN_TELEMETRY_CONFIG_PATH: join(dirname(paths.state_path), "telemetry.json"),
    ...
    const Launch = (
      ...
      Effect.gen(function* () {
        const paths = yield* store
          .Paths()
          .pipe(Effect.mapError((cause) => new ForgeLifecycleError({ cause, code: "timeout" })));
        yield* Effect.forEach(
          [
            ...
            ...(input.config.serve_frontend === true ? [artifact.static_frontend_root] : []),
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* host.ShutdownRequested.pipe(
            ...
            Effect.ensuring(
              RemoveForgeState(paths.state_path, input.instance_id).pipe(Effect.ignore),
            ),
          );
        }),
      );
