...
async fn execute_configured_job(job: Job, shutdown: &Arc<CancelHandle>) -> Execution {
    ...
    // Cursor turns carry no provider continuation in this packet; a
    ...
    // Grok carries its G3 continuation and gates it at the executor below;
    // Hermes carries its gateway continuation below.
    if request.input.continuation.is_some()
        && matches!(
            request.input.launch,
            super::InternalLaunch::Claude(_) | super::InternalLaunch::Cursor(_)
        )
    {
        return request.fail(EngineOperationError::Configuration);
    }
