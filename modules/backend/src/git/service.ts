...
import type { GitStatusSnapshot } from "./model";
import { GitMutationDriver } from "./mutation-driver";
import {
	GitRepository,
...
import { GitReadError, GitReadService } from "./read-service";
...
const normalize_absolute_path = (path: string) => {
	...
	return /^[a-z]:\//u.test(slash_path)
		? `${slash_path.charAt(0).toUpperCase()}${slash_path.slice(1)}`
		: slash_path;
};
	...
	Effect.gen(function* () {
		...
		const RecordEvidence = (
			...
			workspace.repository_state === "not_repository"
				? Effect.void
				: Effect.gen(function* () {
						const current_worktree = workspace.worktrees.find(
							(worktree) => worktree.is_current,
						);
						if (current_worktree === undefined)
							return yield* new GitRepositoryInvariantError({
								message: `Git workspace ${workspace.workspace_id} has no current worktree`,
							});
						return yield* evidence.RecordGitWorkspaceObserved({
							...
							...(trace.raw_origin === undefined
								? {}
								: { raw_origin: trace.raw_origin }),
							root_path: current_worktree.path,
