...
import { GitParseError, ParseGitNumstat, ParseGitStatus, ParseGitWorktrees } from "./parsers";
import {
...
function complete_utf8_prefix_length(bytes: Uint8Array) {
	...
	while (lead_index >= 0 && ((bytes.at(lead_index) ?? 0) & 0b1100_0000) === 0b1000_0000) {
		lead_index -= 1;
	}
	if (lead_index < 0) {
		return 0;
	}
	return bytes.byteLength - lead_index < utf8_sequence_length(bytes.at(lead_index) ?? 0)
		? lead_index
		: bytes.byteLength;
}
