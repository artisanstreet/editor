...
function status_flags(status: string, kind: GitFileStatus["kind"]) {
	...
	const index_status = status.at(0);
	const worktree_status = status.at(1);
	if (index_status === undefined || worktree_status === undefined) {
		throw new Error("Git record contains an incomplete XY status");
	}

	return {
		conflicted,
		index_status,
		staged: !conflicted && !untracked && !ignored && index_status !== ".",
		status,
		untracked,
		unstaged: !conflicted && !untracked && !ignored && worktree_status !== ".",
		worktree_status,
	};
}
