...
import { ComputeContentIdentity, DecodeWorkspaceText, EncodeWorkspaceText } from "./content";
import {
	WorkspaceMutationAuthority,
	WorkspaceMutationAuthorityConflict,
} from "../mutations/authority";
import {
...
import { WorkspaceEvidenceRecorder } from "../evidence";
import {
...
export class WorkspaceFileServiceError extends Data.TaggedError("WorkspaceFileServiceError")<{
	...
	readonly reason: "changed" | "diff_limit" | "failed" | "not_text";
}> {}
		...
		const Read = (query: WorkspaceFileReadQueryValue) =>
			Schema.decodeUnknownEffect(WorkspaceFileReadQuery, { onExcessProperty: "error" })(
				...
				Effect.flatMap((decoded) =>
					Effect.gen(function* () {
						...
						const bytes = yield* reader
							.ReadRegularFile(decoded.path, workspace_text_maximum_bytes)
							.pipe(Effect.mapError(() => failed("read")));
						/**
						 * A file whose bytes are not UTF-8 — a database, an image, any
						 * binary — is reported as such rather than as a generic failure.
						 * The editor tells the reader why the file will not open, and
						 * "could not be completed" never explains that.
						 */
						const content = yield* DecodeWorkspaceText(bytes).pipe(
							Effect.mapError(
								() =>
				...
				Effect.mapError((error) =>
					error instanceof WorkspaceFileServiceError ? error : failed("read"),
				),
			);
