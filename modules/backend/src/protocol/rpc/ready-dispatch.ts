...
import { MakeTelemetryControlHandlers } from "./telemetry-handlers";
...
export class ReadyHandlerFactory extends Context.Service<
  ReadyHandlerFactory,
  {
    readonly Make: Effect.Effect<{
      ...
      readonly workspace_inspection: Effect.Success<typeof MakeWorkspaceInspectionQueryHandler>;
    }>;
  }
...
export const IsReadyConnectionReadEnvelope = (
  ...
  switch (envelope.kind) {
    ...
    case "telemetry.preferences.query":
...
export const MakeReadyEnvelopeDispatch = Effect.gen(function* () {
  ...
  const projection_subscriptions = yield* MakeProjectionSubscriptionHandlers(subscription_control);
  const telemetry = yield* MakeTelemetryControlHandlers;
  ...
  const Handle = (
    ...
    switch (envelope.kind) {
      ...
      case "telemetry.preferences.query":
        return telemetry.HandleQuery(envelope, current);
      case "telemetry.preferences.update":
        return telemetry.HandleUpdate(envelope, current);
      case "telemetry.intent.capture":
        return telemetry.HandleCapture(envelope, current);
      ...
      case "unsubscribe":
        return Effect.die("Unsubscribe admission must capture its exact claim before dispatch.");
      case "ack":
