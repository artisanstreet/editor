...
function cli_error(cause: GitHubCliError, host: string) {
	...
	return provider_error(reasons[cause.reason], cause.retryable, host);
}
...
function installation_error(
	...
	if (installation._tag === "missing") {
		return provider_error("cli_missing", false, host);
	}

	if (installation._tag === "incompatible") {
		return provider_error("cli_incompatible", false, host);
	}

	return installation._tag === "unavailable"
		? provider_error("cli_unavailable", true, host)
		: undefined;
}
...
function DecodeCursor(input: GitProviderDiscoveryInput) {
	...
	return Effect.try(() =>
		...
		Effect.flatMap((cursor) =>
			...
			cursor.account_login !== input.selection.account_login ||
...
function map_repository(host: string, repository: GitHubCliRepository) {
	if (
		repository.name_with_owner !== `${repository.owner}/${repository.name}` ||
		!repository_urls_match_host(host, repository)
	) {
		return Effect.fail(provider_error("invalid_response", false, host));
	}
		...
		Effect.gen(function* () {
			...
			const DiscoverRepositories = (unknown_input: GitProviderDiscoveryInput) =>
				Effect.gen(function* () {
					...
					if (input.selection.provider_id !== github_provider_id) {
						return yield* Effect.fail(
							provider_error("invalid_input", false, input.selection.host),
						);
					}
					...
					const authentication = inspection.authentication.find(
						(candidate) => candidate.host === input.selection.host,
					);

					if (authentication === undefined) {
						return yield* Effect.fail(
							provider_error("unsupported_host", false, input.selection.host),
						);
					}

					if (authentication.active_account._tag === "none") {
						return yield* Effect.fail(
							provider_error("auth_required", false, input.selection.host),
						);
					}

					if (
						authentication.active_account.account_login !==
						input.selection.account_login
					) {
						return yield* Effect.fail(
					...
					if (page.viewer_login !== input.selection.account_login) {
						return yield* Effect.fail(
							provider_error("account_not_active", false, input.selection.host),
						);
					}
				...
				Descriptor: {
					capabilities: [
						...
						{ _tag: "available", capability: "discover_repositories" },
						{
							...
							reason: "Project registration is a later durable milestone",
						},
						{
							_tag: "unavailable",
