...
export const ThreadErasureLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const IsPendingGitCheckout = inArray(WorkspaceGitCheckoutApprovals.state, [
			"requested",
			"approved",
			"executing",
		]);
		const IsPendingGitMutation = inArray(WorkspaceGitMutationApprovals.state, [
			"requested",
			"approved",
			"executing",
			"action_required",
										...
										exists(
											database.client
												...
												.where(
													and(
														eq(
															WorkspaceGitCheckoutApprovals.thread_id,
															Threads.thread_id,
														),
														IsPendingGitCheckout,
													),
												),
										),
									),
										...
										exists(
											database.client
												...
												.where(
													and(
														eq(
															WorkspaceGitMutationApprovals.thread_id,
															Threads.thread_id,
														),
			...
			Effect.gen(function* () {
				...
				const ErasureTransaction = database.client.transaction((transaction) =>
					Effect.gen(function* () {
						...
						const [pending_checkout] = yield* transaction
							.select({ approval_id: WorkspaceGitCheckoutApprovals.approval_id })
							.from(WorkspaceGitCheckoutApprovals)
							.where(
								and(
									eq(WorkspaceGitCheckoutApprovals.thread_id, thread_id),
									IsPendingGitCheckout,
								),
							)
							.limit(1);
						...
						const [pending_git_mutation] = yield* transaction
							...
							.where(
								and(
									eq(WorkspaceGitMutationApprovals.thread_id, thread_id),
									IsPendingGitMutation,
								),
							)
							.limit(1);
						...
						yield* transaction
							.delete(WorkspaceGitCheckoutApprovals)
							.where(eq(WorkspaceGitCheckoutApprovals.thread_id, thread_id));
						yield* transaction
							.delete(WorkspaceGitMutationClaims)
							.where(eq(WorkspaceGitMutationClaims.thread_id, thread_id));
						yield* transaction
							.delete(WorkspaceGitMutationApprovals)
							.where(eq(WorkspaceGitMutationApprovals.thread_id, thread_id));
						yield* transaction.delete(WorkspaceGitSessions).where(
							exists(
								transaction
									.select({ operation_id: WorkspaceGitOperations.operation_id })
									.from(WorkspaceGitOperations)
									.where(
										and(
											eq(WorkspaceGitOperations.thread_id, thread_id),
											eq(
												WorkspaceGitOperations.workspace_id,
												WorkspaceGitSessions.workspace_id,
											),
											eq(
												WorkspaceGitOperations.journal_sequence,
												WorkspaceGitSessions.journal_sequence,
											),
										),
									),
							),
