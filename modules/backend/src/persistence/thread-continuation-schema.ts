...
export const ThreadPortableHandoffs = sqliteTable(
	...
	(table) => [
		...
		/** Legacy native-extraction method names remain valid for existing rows. */
		check(
			"thread_portable_handoffs_method_check",
			sql`${table.method} IN ('compaction_model_summary', 'canonical_transcript_summary', 'claude_post_compact', 'codex_fork_summary')`,
		),
		check(
