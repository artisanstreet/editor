...
import { ProductTelemetry, ProductTelemetryNoop } from "../telemetry/product-telemetry";
import {
  TelemetryPreferencesControl,
  TelemetryPreferencesControlNoop,
} from "../telemetry/preferences-control";
...
export interface BackendOptions {
  ...
  /** Product analytics boundary; omitted is a total no-op in development and tests. */
...
export function make_backend_layer(options: BackendOptions) {
  ...
  const product_telemetry = options.product_telemetry ?? ProductTelemetryNoop;
  const telemetry_preferences = options.telemetry_preferences ?? TelemetryPreferencesControlNoop;
  ...
  const guidance = make_global_guidance_service_layer({
    backups_directory: options.guidance?.backups_directory ?? join(guidance_directory, "backups"),
  ...
  /**
   * Declared before orchestration because observed engine shells and native
   * PTYs must feed one terminal service and one scrollback registry.
   */
  ...
  const orchestration = AgentOrchestratorLive.pipe(
    ...
    Layer.provideMerge(product_telemetry),
  );
  ...
  const preview_browser = (options.preview_external_browser ?? NodePreviewExternalBrowserLive).pipe(
    Layer.provide(preview_targets),
  );
  ...
  const protocol_foundation = protocol_foundation_base.pipe(
    ...
    Layer.provideMerge(product_telemetry),
    Layer.provideMerge(telemetry_preferences),
 ...
 */
const production_engine_ids: ReadonlySet<string> = new Set([
  "codex",
  "claude",
  "opencode2",
  "grok",
...
export function make_desktop_backend_layer(options: DesktopBackendOptions) {
  ...
  return Layer.unwrap(
    Effect.gen(function* () {
      if (
        (options.engines ?? []).some((engine) => !production_engine_ids.has(engine.Descriptor.id))
      ) {
      ...
      return make_backend_layer({
        ...
        wake_lock_backend:
          options.wake_lock_backend ?? make_platform_wake_lock_backend_layer(process.platform),
      });
