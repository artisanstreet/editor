...
impl Repository {
    ...
    pub async fn project_steered_message(
        ...
        {
            let state = entities::conversation_state::Entity::find_by_id(
                command.thread_id.as_str(),
            )
            .one(&transaction)
            .await
            .map_err(|source| {
                RunLaunchError::Repository(database_error("read steered cursor state", source))
            })?
            .ok_or(RunLaunchError::Repository(RepositoryError::Invariant {
                reason: "projected steered item references missing conversation state",
            }))?;
            let cursor_value = u64::try_from(state.last_patch_sequence).map_err(|_| {
                counter_overflow("patch sequence", state.last_patch_sequence)
            })?;
        ...
        let completed = transaction
            .execute_raw(Statement::from_sql_and_values(
                DbBackend::Sqlite,
                super::message_dispatch::COMPLETE_STEERED_SQL,
                [
                    operated_at_ms.into(),
            ...
            .rows_affected();
        if completed != 1 {
            match super::message_dispatch::classify_steered_transition_miss(
                &transaction,
                command.message_id,
            )
            .await
            {
                Ok(super::message_dispatch::SteeredRowTerminal::Completed(_)) => {
                    return Err(RunLaunchError::Repository(RepositoryError::Invariant {
                        reason: "steered dispatch completed without its projected item",
                    }));
                }
                Ok(super::message_dispatch::SteeredRowTerminal::Failed(_)) => {
                    return Err(RunLaunchError::Repository(
