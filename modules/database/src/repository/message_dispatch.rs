...
impl Repository {
    ...
    pub async fn fail_steered_dispatch(
        ...
        let reason = DispatchFailureReason::parse(reason).map_err(|_| {
    ...
    pub async fn fail_orphaned_steered_dispatches(
        ...
        let reason = DispatchFailureReason::parse("steer target run is no longer live").map_err(
            |_| RepositoryError::Invariant {
    ...
    pub async fn renew_message_dispatch_lease(
        ...
        let transaction = self.database.begin().await.map_err(|source| {
            database_error("begin message-dispatch lease renewal", source)
        })?;
        let statement = Statement::from_sql_and_values(
            DbBackend::Sqlite,
        ...
        let Some(row) = returned else {
            let rejection = classify_unfenced_lease_renewal(
                &transaction,
                message_id,
                owner,
                operated_at_ms,
        ...
        let transitioned = match renewed_from_row(
            &row,
            message_id,
            operated_at_ms,
            lease_expires_at_ms,
            ...
            Condition::any()
                .add(
                    Condition::all()
                        ...
                        .add(
                            entities::message_dispatch::Column::AvailableAtMs.lte(claimed_at_ms),
                        ),
                )
                .add(
                    Condition::all()
                        ...
                        .add(
                            entities::message_dispatch::Column::LeaseExpiresAtMs
                                .lte(claimed_at_ms),
                        ),
                ),
        )
