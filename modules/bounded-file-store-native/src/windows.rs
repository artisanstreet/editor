...
pub(crate) struct RootHandle(HANDLE);

/** The Windows handle is an owned kernel resource and is safe to lease across the async task. */
unsafe impl Send for RootHandle {}

/** The Windows handle is immutable after opening and can be shared by read-only child opens. */
...
pub(crate) fn open_root(root_directory: &str) -> Result<RootHandle> {
    ...
    let handle =
        RootHandle(result.map_err(|_| native_error("native file store root is unavailable"))?);
