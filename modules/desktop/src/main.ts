...
import { InitializeDesktopSentry } from "./sentry";
...
const desktop_release_version =
  typeof __ARTISAN_RELEASE_VERSION__ === "undefined" ? "development" : __ARTISAN_RELEASE_VERSION__;
const desktop_release_commit =
  typeof __ARTISAN_RELEASE_COMMIT__ === "undefined" ? "development" : __ARTISAN_RELEASE_COMMIT__;
const desktop_sentry = InitializeDesktopSentry({
  config: {
    dsn:
      process.env.ARTISAN_SENTRY_EDITOR_DSN ??
      (typeof __ARTISAN_SENTRY_EDITOR_DSN__ === "undefined"
        ? undefined
        : __ARTISAN_SENTRY_EDITOR_DSN__),
    environment:
      process.env.ARTISAN_TELEMETRY_ENVIRONMENT ??
      (typeof __ARTISAN_TELEMETRY_ENVIRONMENT__ === "undefined"
        ? "development"
        : __ARTISAN_TELEMETRY_ENVIRONMENT__),
    release: `artisan-editor@${desktop_release_version}+${desktop_release_commit}`,
  },
  preferences_path: process.env.ARTISAN_TELEMETRY_CONFIG_PATH,
  sdk: {
...
/**
 * Perfetto's binary ring expands several-fold when Electron serializes it to
 * JSON. The previous 64 MiB ring produced 230–307 MiB crash artifacts while
 * the main process was already recovering an OOM renderer; 16 MiB retains the
 * final failure window without adding another large allocation spike.
 */
const trace_buffer_size_in_kb = 16 * 1024;
 ...
 * HTTP/WS like a paired browser; there is no preload or IPC surface. Its only
 * shell-owned preference is the allowlisted runtime icon selected below.
 */
export const StartDesktop = Effect.gen(function* () {
  ...
  const app_icon_root = normalize(join(import.meta.dirname, "app-icons"));
  const app_icon_preference_path = join(app.getPath("userData"), "app-icon.json");
  const windows_app_icon_root = join(app.getPath("userData"), "app-icons");
  let selected_app_icon = yield* LoadDesktopAppIconPreference(app_icon_preference_path);
  const AppIconFor = (icon: DesktopAppIconPreference) =>
    Effect.gen(function* () {
      const bytes = yield* Effect.tryPromise({
        catch: (cause) => cause,
        try: () => readFile(join(app_icon_root, desktop_app_icon_assets[icon].image)),
      });
      const image = nativeImage.createFromBuffer(bytes);
      if (image.isEmpty()) return yield* Effect.fail(`Packaged app icon is missing: ${icon}`);
      const windows_shell_path =
        process.platform === "win32"
          ? yield* MaterializeWindowsAppIcon(app_icon_root, windows_app_icon_root, icon)
          : undefined;
      return { image, windows_shell_path };
    });
  const initial_app_icon = yield* AppIconFor(selected_app_icon);
  ...
  /**
   * Explorer caches a taskbar group's artwork by AppUserModelID. Updating
   * WM_SETICON and RelaunchIconResource under the same ID only refreshes the
   * thumbnail; moving an alternative onto its own window-level identity makes
   * Explorer rebuild the actual button. The process keeps the product ID above
   * so toast activation and the installed shortcut retain their stable identity.
   */
  const WindowsTaskbarAppIdFor = (icon: DesktopAppIconPreference) =>
    icon === default_desktop_app_icon
      ? windows_app_user_model_id
      : `${windows_app_user_model_id}.${icon}`;
  const SetWindowsTaskbarIcon = (icon: DesktopAppIconPreference, path: string) => {
    editor_window.setAppDetails({
      appIconIndex: 0,
      appIconPath: path,
      appId: WindowsTaskbarAppIdFor(icon),
      relaunchCommand: process.execPath,
      relaunchDisplayName: "Artisan Editor",
    });
  };
  if (process.platform === "darwin") app.dock?.setIcon(initial_app_icon.image);
  if (process.platform === "win32" && initial_app_icon.windows_shell_path !== undefined) {
    SetWindowsTaskbarIcon(selected_app_icon, initial_app_icon.windows_shell_path);
  }
  const ApplyAppIcon = (icon: DesktopAppIconPreference) =>
    Effect.gen(function* () {
      const selected = yield* AppIconFor(icon);
      yield* SaveDesktopAppIconPreference(app_icon_preference_path, icon);
      yield* Effect.try({
        catch: (cause) => cause,
        try: () => {
          if (process.platform === "darwin") app.dock?.setIcon(selected.image);
          else editor_window.setIcon(selected.image);
          if (process.platform === "win32" && selected.windows_shell_path !== undefined) {
            SetWindowsTaskbarIcon(icon, selected.windows_shell_path);
          }
        },
      });
  ...
  renderer_session.protocol.handle(app_scheme, (request) =>
    Effect.runPromise(
      Effect.gen(function* () {
        const control_response = yield* HandleDesktopAppIconRequest(request, {
          Current: () => selected_app_icon,
          Select: ApplyAppIcon,
        });
        if (Option.isSome(control_response)) return control_response.value;
        return yield* ServeRendererAsset(frontend_root, request.url);
      }),
    ),
  );
  /**
   * The renderer owns the document title; the window keeps its fixed name.
   * With no preload or IPC, that pinned title remains the ambient status
   * channel: the needs-attention marker the renderer embeds in it becomes the
   * taskbar badge here, as a numbered overlay where Windows wants an image and
   * the native dock badge everywhere else. The app-icon endpoint above is a
   * separate, explicit command restricted to two packaged values.
   */
  /**
  ...
  if (renderer_diagnostics_enabled) {
    ...
    const RunRendererHeartbeat = () => {
      if (!renderer_loaded || heartbeat_in_progress || editor_window.webContents.isDestroyed())
        return;
    ...
    const heartbeat_interval = setInterval(RunRendererHeartbeat, renderer_heartbeat_interval_ms);
    ...
    editor_window.webContents.on("before-input-event", (event, input) => {
      if (input.type !== "keyDown" || !input.control || !input.shift || input.key !== "F9") return;
      event.preventDefault();
  ...
  const renderer_death_recovery = make_renderer_death_recovery({
    ...
    ReportFailure: () => {
      desktop_sentry.capture("forge_recovery_failed", {});
      console.error(
        JSON.stringify({
  ...
  editor_window.webContents.on("unresponsive", () => {
    desktop_sentry.capture("renderer_unresponsive", {});
  });
  editor_window.webContents.on("render-process-gone", (_event, details) => {
    desktop_sentry.capture("renderer_gone", { renderer_reason: details.reason });
    renderer_death_recovery.Request();
  });
  const Cleanup = () =>
    (cleanup ??= Effect.runPromise(
      renderer_death_recovery.Close().pipe(
        Effect.andThen(desktop_lifecycle.Cleanup()),
        Effect.andThen(
          Effect.promise(async () => {
            await desktop_sentry.flush();
          }),
        ),
      ),
    ));
...
void Effect.runPromise(StartDesktop).catch((cause) => {
  desktop_sentry.capture("bootstrap_failed", {});
  console.error(
    JSON.stringify({
