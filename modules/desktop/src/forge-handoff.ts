 ...
 */
export const make_node_forge_handoff_process_layer = Layer.succeed(
	ForgeHandoffProcess,
	ForgeHandoffProcess.of({
		Request: (ae_command_path) =>
			Effect.callback<ForgeHandoff, DesktopLauncherError>((resume) => {
				...
				const on_error = (cause: Error) =>
					complete(
						Effect.fail(new DesktopLauncherError({ cause, reason: "handoff_failed" })),
					);
		...
		StopOwned: (ae_command_path, instance_id) =>
			Effect.callback<void, DesktopLauncherError>((resume) => {
				const child = spawn(ae_command_path, OwnedForgeStopArguments(instance_id), {
					shell: IsWindowsCommandScript(ae_command_path),
					stdio: "ignore",
					windowsHide: true,
				});
			...
			return DesktopForgeLifecycle.of({
				Cleanup: () =>
					Effect.gen(function* () {
						...
						const in_flight = current.reconnect_in_flight;
						if (Option.isSome(in_flight)) {
							yield* Deferred.await(in_flight.value).pipe(
