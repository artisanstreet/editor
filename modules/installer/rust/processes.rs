...
pub struct RetirementPolicy {
    ...
    pub force: bool,
    /// Closes superseded Editors before gating Forge, so a Forge an Editor
    /// owns stops with it the way it does when its window closes. For
    /// callers whose intent is to replace the running Editor (the dev
    /// relaunch); a Forge that outlives its Editors is still gated exactly
    /// as without this option.
    pub close_editors_first: bool,
}
...
pub fn retire_superseded(
    ...
    let (forges, editors) = if policy.close_editors_first && !editors.is_empty() {
        (
            retire_editors_first(&versions_root, forges, &editors, &mut retirement)?,
            Vec::new(),
        )
    } else {
        (forges, editors)
    ...
    if !editors.is_empty() {
        close_editors(&versions_root, &editors)?;
...
/// Closes the Editors, then returns the Forges still running afterwards:
/// owned Forges leave with their Editor, and only survivors are gated.
fn retire_editors_first<'a>(
    versions_root: &Path,
    forges: Vec<&'a RunningProcess>,
    editors: &[&RunningProcess],
    retirement: &mut Retirement,
) -> Result<Vec<&'a RunningProcess>> {
    close_editors(versions_root, editors)?;
    retirement.editors_closed = editors.len();
    let alive = await_exit(versions_root, &forges, GRACEFUL_WINDOW)?;
    retirement.forges_stopped = forges.len() - alive.len();
    Ok(forges
        .into_iter()
        .filter(|process| alive.iter().any(|survivor| same_process(survivor, process)))
        .collect())
}

/// Asks each Editor to close, then ends any that did not within the window.
fn close_editors(versions_root: &Path, editors: &[&RunningProcess]) -> Result<()> {
    for process in editors {
        request_close(process.pid);
    }
    for process in await_exit(versions_root, editors, GRACEFUL_WINDOW)? {
        terminate(process.pid);
    }
    Ok(())
}
