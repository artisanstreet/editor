...
//!
//! Scope is deliberately narrow: resolve the `claude` executable from an
//! explicit override, the `ARTISAN_CLAUDE_EXECUTABLE` environment value, or
//! `PATH`, then run only the bounded, non-billable `--version` and
...
pub use discovery::{
    CLAUDE_DEFAULT_COMMAND, CLAUDE_EXECUTABLE_ENV_VAR, ClaudeDiscoveryError, ClaudeExecutable,
    ClaudeExecutableSource, MAX_EXECUTABLE_ARG_BYTES, MAX_EXECUTABLE_ARGS,
    MAX_EXECUTABLE_VALUE_BYTES, candidate_file_names, discover_claude_executable, search_path_for,
