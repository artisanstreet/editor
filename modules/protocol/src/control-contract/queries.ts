...
import { RuntimeCatalog, RuntimeCatalogScope } from "../runtime-catalog";
...
import { TelemetryIntent, TelemetryPreferences, TelemetryPreferencesUpdate } from "../telemetry";
...
/** Requests a capability catalog, optionally scoped to live engine inventory. */
export const RuntimeCatalogQueryEnvelope = Schema.Struct({
  ...NegotiatedFrontendTraceMetadata,
  kind: Schema.Literal("runtime.catalog.query"),
  payload: Schema.Struct({
    profile_id: Schema.optional(RuntimeCatalogScope.fields.profile_id),
    working_directory: Schema.optional(RuntimeCatalogScope.fields.working_directory),
    workspace_trust: Schema.optional(RuntimeCatalogScope.fields.workspace_trust),
  }),
});
...
/** Requests the current independent telemetry consent choices. */
export const TelemetryPreferencesQueryEnvelope = Schema.Struct({
  ...NegotiatedFrontendTraceMetadata,
  kind: Schema.Literal("telemetry.preferences.query"),
  payload: Schema.Struct({}),
});
export type TelemetryPreferencesQueryEnvelope = typeof TelemetryPreferencesQueryEnvelope.Type;

/** Returns the current telemetry choices without exposing installation identity. */
export const TelemetryPreferencesQueryResultEnvelope = Schema.Struct({
  ...NegotiatedBackendTraceMetadata,
  correlation_id: Identifier,
  kind: Schema.Literal("telemetry.preferences.query.result"),
  payload: TelemetryPreferences,
});
export type TelemetryPreferencesQueryResultEnvelope =
  typeof TelemetryPreferencesQueryResultEnvelope.Type;

/** Updates one or both telemetry choices through a closed, non-empty patch. */
export const TelemetryPreferencesUpdateEnvelope = Schema.Struct({
  ...NegotiatedFrontendTraceMetadata,
  kind: Schema.Literal("telemetry.preferences.update"),
  payload: TelemetryPreferencesUpdate,
});
export type TelemetryPreferencesUpdateEnvelope = typeof TelemetryPreferencesUpdateEnvelope.Type;

/** Returns the authoritative telemetry choices after an update. */
export const TelemetryPreferencesUpdateResultEnvelope = Schema.Struct({
  ...NegotiatedBackendTraceMetadata,
  correlation_id: Identifier,
  kind: Schema.Literal("telemetry.preferences.update.result"),
  payload: TelemetryPreferences,
});
export type TelemetryPreferencesUpdateResultEnvelope =
  typeof TelemetryPreferencesUpdateResultEnvelope.Type;

/** Carries one closed renderer-only telemetry intent to Forge. */
export const TelemetryIntentCaptureEnvelope = Schema.Struct({
  ...NegotiatedFrontendTraceMetadata,
  kind: Schema.Literal("telemetry.intent.capture"),
  payload: TelemetryIntent,
});
export type TelemetryIntentCaptureEnvelope = typeof TelemetryIntentCaptureEnvelope.Type;

/** Acknowledges local acceptance without returning vendor state or identity. */
export const TelemetryIntentCaptureResultEnvelope = Schema.Struct({
  ...NegotiatedBackendTraceMetadata,
  correlation_id: Identifier,
  kind: Schema.Literal("telemetry.intent.capture.result"),
  payload: Schema.Struct({ accepted: Schema.Literal(true) }),
});
export type TelemetryIntentCaptureResultEnvelope = typeof TelemetryIntentCaptureResultEnvelope.Type;
