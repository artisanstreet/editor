...
/** Declares the deliberately limited, truthful Claude Code V1 capability surface. @since 0.4.0 */
export const ClaudeEngineDescriptor: EngineDescriptor = {
	capabilities: {
		approval: {
			state: "unsupported",
			reason: "V1 does not expose an approval response channel.",
		},
		auth: {
			state: "supported",
		...
		question: {
			state: "unsupported",
			reason: "Claude Code V1 has no canonical question response mapping here.",
		},
		...
		subagents: { state: "unsupported", reason: "Subagent events are not canonicalized in V1." },
	},
...
/** Configures Claude CLI execution and all bounded process/protocol limits. @since 0.4.0 */
...
/** Provides the Claude Code engine service. @since 0.4.0 */
...
function validate_provider_options(input: EngineOpenInput) {
	...
	const allowed = new Set(["claude.permission_mode", "claude.append_system_prompt_file"]);
	...
	if (
		...
		(typeof permission_mode !== "string" ||
			!["default", "acceptEdits", "plan", "dontAsk"].includes(permission_mode))
	) {
...
function make_probe(
	...
	return Effect.gen(function* () {
		...
		const auth_status = (() => {
			try {
				return Schema.decodeUnknownOption(Schema.Struct({ loggedIn: Schema.Boolean }))(
					JSON.parse(new TextDecoder().decode(auth_stdout)),
				);
			} catch {
				return undefined;
			}
		})();
		const unknown = auth_status === undefined || auth_exit.code !== 0;
		const authenticated =
			!unknown &&
			auth_status !== undefined &&
			auth_status._tag === "Some" &&
			auth_status.value.loggedIn;
...
/** Builds a Claude Code engine using only the installed CLI and subscription session. @since 0.4.0 */
