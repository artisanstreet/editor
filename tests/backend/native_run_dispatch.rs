...
async fn dispatch_fixture_midturn_engine_loss_recovers_without_second_spawn() {
    ...
    // Windows terminates the configured job before waiting. Unix first lets
    // the fixture exit through its closed lifeline, and kills only on timeout.
    #[cfg(windows)]
    assert_eq!(loss_counts.kills_requested, 1);
    #[cfg(not(windows))]
    assert!(loss_counts.kills_requested <= 1);
