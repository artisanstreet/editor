...
fn view_cursor(
    ...
    id: &ThreadId,
) -> Option<ConversationCursor> {
    registry.view(id).map(|view| view.cursor())
}
...
fn view_state(
    ...
    registry.view(id).map(|view| view.state())
}
...
fn empty_registry_has_no_entries_and_first_pending_lease_is_nonzero_at_cursor_zero() {
    ...
    let lease = registry
        .register_pending(thread("thread-1"), cursor(0))
    ...
    assert_eq!(view_cursor(&registry, &thread("thread-1")), Some(cursor(0)));
    assert_eq!(
        view_state(&registry, &thread("thread-1")),
...
fn distinct_threads_coexist_independently() {
    ...
    let lease_a = registry
        .register_pending(thread("thread-a"), cursor(0))
        .expect("thread a should register");
    let lease_b = registry
        .register_pending(thread("thread-b"), cursor(5))
    ...
    assert_eq!(view_cursor(&registry, &thread("thread-a")), Some(cursor(0)));
    assert_eq!(view_cursor(&registry, &thread("thread-b")), Some(cursor(5)));

    // Activate only a.
    let returned = registry.activate(&lease_a).expect("activate a");
    assert_eq!(returned, cursor(0));
    assert_eq!(
        view_state(&registry, &thread("thread-a")),
        Some(SubscriptionState::Active)
    );
    assert_eq!(
        view_state(&registry, &thread("thread-b")),
        Some(SubscriptionState::Pending)
    );
    ...
    registry
        .publish_batch(&lease_a, &batch_a)
        .expect("batch for a should advance");
    assert_eq!(view_cursor(&registry, &thread("thread-a")), Some(cursor(1)));
    assert_eq!(view_cursor(&registry, &thread("thread-b")), Some(cursor(5)));
}
...
fn pending_activation_returns_exact_cursor_and_second_activation_is_rejected() {
    let mut registry = ConversationSubscriptionRegistry::new();
    let lease = registry
        .register_pending(thread("thread-1"), cursor(7))
    ...
    let before_view = registry.view(&thread("thread-1"));
    let before_len = registry.len();
    let before_empty = registry.is_empty();
    let err = registry
        .activate(&lease)
        .expect_err("second activation should fail");
    assert_eq!(err, ActivateError::AlreadyActive);
    assert_eq!(registry.view(&thread("thread-1")), before_view);
    assert_eq!(registry.len(), before_len);
    assert_eq!(registry.is_empty(), before_empty);
    ...
    assert_eq!(view_cursor(&registry, &thread("thread-1")), Some(cursor(8)));
}
...
fn replacement_of_pending_entry_mints_newer_lease_and_stales_old_one() {
    ...
    let lease_new = registry
        .register_pending(thread("thread-1"), cursor(3))
        .expect("replacement pending");

    assert!(lease_new.generation().get() > lease_old.generation().get());
    assert_eq!(view_cursor(&registry, &thread("thread-1")), Some(cursor(3)));
    assert_eq!(
        view_state(&registry, &thread("thread-1")),
        Some(SubscriptionState::Pending)
    );
    ...
    let before_view2 = registry.view(&thread("thread-1"));
    assert_eq!(
        registry.publish_batch(&lease_old, &batch),
        Err(ApplyBatchError::StaleLease)
    );
    assert_eq!(registry.view(&thread("thread-1")), before_view2);
    ...
    registry
        .publish_batch(&lease_new, &batch)
        .expect("new lease should publish");
    assert_eq!(view_cursor(&registry, &thread("thread-1")), Some(cursor(4)));
}
...
fn replacement_of_active_entry_mints_newer_pending_and_stales_old_active_lease() {
    ...
    let lease_active = registry
        .register_pending(thread("thread-1"), cursor(10))
    ...
    registry
        .publish_batch(&lease_active, &batch1)
        .expect("advance to 11");

    let lease_new = registry
        .register_pending(thread("thread-1"), cursor(99))
    ...
    assert_eq!(
        view_cursor(&registry, &thread("thread-1")),
        Some(cursor(99))
    );
...
fn unsubscribe_pending_and_active_and_unknown_and_removed_lease_rejection() {
    ...
    let lease_pending = registry
        .register_pending(thread("thread-pending"), cursor(1))
        .expect("pending");
    let lease_active = registry
        .register_pending(thread("thread-active"), cursor(2))
    ...
    assert!(registry.view(&thread("thread-pending")).is_none());
    assert_eq!(registry.len(), 1);
...
fn matching_active_patch_batch_advances_exactly_from_to() {
    ...
    assert_eq!(view_cursor(&registry, &thread("thread-1")), Some(cursor(2)));
}
...
fn pending_publication_is_rejected_without_mutation() {
    let mut registry = ConversationSubscriptionRegistry::new();
    let lease = registry
        .register_pending(thread("thread-1"), cursor(4))
...
fn wrong_thread_lease_and_batch_are_rejected_with_preservation() {
    ...
    let lease_a = registry
        ...
        .expect("a");
    let lease_b = registry
        .register_pending(thread("thread-b"), cursor(0))
        .expect("b");
    registry.activate(&lease_a).expect("activate a");
    registry.activate(&lease_b).expect("activate b");

    let before_a = registry.view(&thread("thread-a"));
    let before_b = registry.view(&thread("thread-b"));
    let before_len = registry.len();
    // Lease for a with batch for b (thread mismatch).
    let batch_b = batch(thread("thread-b"), 0, 1, vec![append_patch("p-b-1", 1)]);
    assert_eq!(
        registry.publish_batch(&lease_a, &batch_b),
        Err(ApplyBatchError::ThreadMismatch)
    );
    assert_eq!(registry.view(&thread("thread-a")), before_a);
    assert_eq!(registry.view(&thread("thread-b")), before_b);
    assert_eq!(registry.len(), before_len);
    ...
    assert_eq!(
        registry.publish_batch(&lease_b, &batch_a),
        Err(ApplyBatchError::ThreadMismatch)
    );
    assert_eq!(registry.view(&thread("thread-a")), before_a);
    assert_eq!(registry.view(&thread("thread-b")), before_b);
...
fn duplicate_regression_and_gap_are_rejected_with_equality_preservation() {
    ...
    registry
        .publish_batch(&lease, &batch_advance)
        .expect("advance");
    assert_eq!(
        view_cursor(&registry, &thread("thread-1")),
        Some(cursor(12))
    ...
    assert_eq!(
        registry.publish_batch(&lease, &duplicate),
    ...
    assert_eq!(
        registry.publish_batch(&lease, &regression),
        Err(ApplyBatchError::CursorMismatch {
            expected: cursor(12),
            actual: cursor(5)
        })
    ...
    assert_eq!(
        registry.publish_batch(&lease, &gap),
        Err(ApplyBatchError::CursorMismatch {
            expected: cursor(12),
            actual: cursor(13)
        })
...
fn sequential_batches_advance_contiguously_and_never_affect_another_thread() {
    ...
    let lease_a = registry
        ...
        .expect("a");
    let lease_b = registry
        .register_pending(thread("thread-b"), cursor(100))
        .expect("b");
    registry.activate(&lease_a).expect("activate a");
    registry.activate(&lease_b).expect("activate b");
    ...
    for seq in 1..=5 {
        ...
        registry.publish_batch(&lease_a, &batch_a).expect("a batch");
        assert_eq!(
            view_cursor(&registry, &thread("thread-a")),
            Some(cursor(seq))
        );
        assert_eq!(
            view_cursor(&registry, &thread("thread-b")),
            Some(cursor(100)),
    ...
    for seq in 101..=103 {
        ...
        registry.publish_batch(&lease_b, &batch_b).expect("b batch");
        assert_eq!(
            view_cursor(&registry, &thread("thread-b")),
            Some(cursor(seq))
        );
    ...
    registry.publish_batch(&lease_a, &next_a).expect("a next");
    assert_eq!(view_cursor(&registry, &thread("thread-a")), Some(cursor(6)));
}
...
// ---------------------------------------------------------------------------
// Equality and clone semantics (single-copy authority)
// ---------------------------------------------------------------------------

#[test]
fn registry_equality_is_deterministic_without_cloning_the_authority() {
    // Build two registries independently through the same generation history.
    let mut registry = ConversationSubscriptionRegistry::new();
    let lease = registry
        .register_pending(thread("thread-1"), cursor(3))
        .expect("register");
    ...
    let mut other = ConversationSubscriptionRegistry::new();
    let other_lease = other
        .register_pending(thread("thread-1"), cursor(3))
        .expect("register other");
    other.activate(&other_lease).expect("activate other");
    assert_eq!(registry, other);

    let batch = batch(thread("thread-1"), 3, 4, vec![append_patch("p4", 4)]);
    other
        .publish_batch(&other_lease, &batch)
        .expect("publish other");
    assert_ne!(registry, other);
    assert_eq!(
        other.view(&thread("thread-1")).map(|v| v.cursor()),
        Some(cursor(4))
    );
    assert_eq!(
        registry.view(&thread("thread-1")).map(|v| v.cursor()),
        Some(cursor(3))
    );
}

#[test]
fn lease_and_view_are_cloneable_and_equality_testable() {
    let mut registry = ConversationSubscriptionRegistry::new();
    let lease = registry
        .register_pending(thread("thread-x"), cursor(0))
        .expect("register");
    ...
    let view = registry
        .view(&thread("thread-x"))
        .expect("view should exist")
        .clone();
    let view_clone = view.clone();
    assert_eq!(view, view_clone);
    assert_eq!(view_clone.cursor(), cursor(0));
}

#[test]
fn publish_batch_advances_only_after_successful_publication() {
    let mut registry = ConversationSubscriptionRegistry::new();
    let lease = registry
        .register_pending(thread("thread-alias"), cursor(0))
        .expect("register");
    registry.activate(&lease).expect("activate");
    let batch = batch(
        thread("thread-alias"),
