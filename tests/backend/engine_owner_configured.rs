...
//! existing fixture child through the `#[cfg(test)]`-only fixture seam.
...
use super::{
    EngineOwner, EngineOwnerHealth, EngineOwnerShutdown, FixtureConfiguredLaunch, FixtureTurnInput,
...
async fn configured_fixture_cancellation_before_authorize_aborts() {
    ...
    assert!(
        auth_res.is_ok(),
        "authorize send should succeed even after cancel; owner handles cancellation"
    );
