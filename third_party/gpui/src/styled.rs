...
pub trait Styled: Sized {
    ...
    /// Sets a backdrop blur applied behind this element, painted before its background.
    ///
    /// Corners come from the element's own corner radii.
...
mod tests {
    ...
    #[test]
    fn backdrop_blur_sets_the_style_field() {
        let refined = StyleRefinement::default().backdrop_blur(px(8.));
        assert_eq!(refined.backdrop_blur, Some(px(8.)));

        let refined = StyleRefinement::default().backdrop_blur(12.5);
        assert_eq!(refined.backdrop_blur, Some(px(12.5)));
    }
}
