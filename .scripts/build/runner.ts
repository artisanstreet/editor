...
import { launch_installed_editor } from "./launch-editor.ts";
...
const force_retirement = process.argv.slice(2).includes("--force");
const release_commit =
  process.env.ARTISAN_RELEASE_COMMIT ??
  execFileSync("git", ["rev-parse", "HEAD"], {
    cwd: repository_root,
    encoding: "utf8",
  }).trim();

const installer_retirement_arguments = () =>
  force_retirement ? (["--yes", "--force"] as const) : (["--yes"] as const);
  ...
  [
    {
      ...
      steps: [
        {
          name: "lifecycle binaries",
          run: "cargo build --release -p artisan-editor-cli -p ae-installer",
        },
        {
          ...
          run: () =>
            Checklist.command([
              ...
              "prepare-update",
              ...installer_retirement_arguments(),
            ]),
        },
      ],
    },
    ...
    { name: "native build", run: "pnpm run build:native" },
    {
      env: (step) => {
        ...
        return {
          ARTISAN_RELEASE_COMMIT: release_commit,
    ...
    { name: "release server", provides: release_origin, run: () => ServeRelease },
    {
      ...
      run: (step) =>
        Checklist.command([
           ...
           */
          "--yes",
          ...(force_retirement ? ["--force"] : []),
        ]),
    },
    {
      name: "open editor",
      run: () => launch_installed_editor(join(detect_install_root(), "bin", "ae.exe")),
    },
  ],
