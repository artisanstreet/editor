...
- **Backend moving:** a contract exists in the current dirty worktree but is being implemented by the backend session. Consume it only after that session records branch continuity in [`.memory/active-handoff.md`](../../.memory/active-handoff.md) and verified product status in the [`backend-completion-matrix.md`](../status/backend-completion-matrix.md).
...
**Status: Ready.** Use [`barekey-design-language.md`](../design/barekey-design-language.md) as research, not source code.
