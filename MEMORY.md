...
- [x] Latest verified implementation checkpoint: `12173f2 feat: expose local git fetch controls`
- [x] Local `HEAD`, its upstream, and `origin/codex/workspace-replace-approval` must be equal after every checkpoint push and before handoff; this was last verified on 2026-07-14 at `12173f2 feat: expose local git fetch controls`.
...
- [x] Typed preview-target protocol and the durable SQLite core are committed and pushed at `fe4edf5` and `ac4b5d5`. Exact project/workspace scope, canonical journal replay, process-safe probe leases, expiry takeover, source-safe errors, replacement-generation fencing, a transactional 256-target bound, and thread-erasure claim cleanup are covered by 21 focused tests. TypeScript, root Oxlint, scoped formatting, repeated concurrency runs, and independent review pass locally with no P0-P2 finding. Production ProtocolServer, runtime, and `ArtisanClient` integration is the active slice; no remote runner or workflow is used.
