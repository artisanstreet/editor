...
- [x] Replaced the static thread identity with a local-only model selector built from the Barekey popover, tabs, and ScrollArea primitives plus a semantic table. Its compact icon tabs cover Codex, Claude Code, Grok, OpenCode, Cline, and Google's current Antigravity naming; model rows reuse the compact icon/name/lab hierarchy and close the popover after selection.
- [x] The selector is explicitly a frontend prototype and does not restore non-Codex production adapters or claim backend availability. Codex is the default and its lab label is `Codex`, not `OpenAI`. SVGL supplies all requested brand marks except Cline, which uses the honest generic Tabler terminal glyph because `@selemondev/svgl-svelte@2.17.0` has no Cline export.
