...
- [x] Changed the Popover content primitive fundamentally: its default surface now uses the Barekey `card` utility and no longer includes the shadcn ring or `shadow-2xl`. The selector-specific `card!` override was removed, so every popover inherits the correct design-system surface without fighting base styles.
