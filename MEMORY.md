...
- [x] Latest verified implementation checkpoint: `f300468 feat: persist git mutation approvals`
- [x] Local `HEAD`, upstream, and `origin/codex/workspace-replace-approval` were equal at `f300468` after the local full-repository validation passed on 2026-07-14.
...
- [ ] Next backend slice: compose a generic Git mutation coordinator that prepares before approval, executes once, persists attempt before reconciliation, recovers without re-executing, projects the resulting session, and then publishes typed protocol and `ArtisanClient` routes.
