...
## M5 Orchestration And Canonical Surfaces

- [x] Added durable pre-execution intake risk and assumptions, including the ordinary proceed path and clarified-message resolution, plus authoritative renderer session reload/live projections.
- [x] Made steering outcomes final-only and idempotent: successful delivery appends one transactional `thread.message_routed`; disabled, unsupported/experimental, ambiguous-target, rejected, and provider-failure fallbacks append one queued final with truthful reasons.
- [x] Added authoritative transcript history/live reload and thread-scoped orchestration-group discovery through `ArtisanClient`; the renderer does not reconstruct either from local fixtures.
- [x] Added the complete provider-neutral Work/Time/Guidance/Routines/Capabilities/Processes/Changes/Permissions/native-action taxonomy, durable surface projections, optional usage aggregation, atomic snapshot watermarks, scope filtering, reconnect, and erasure invalidation while retaining raw origin.
- [x] Added attributed user/graph workspace review semantics, strict active-reviewer authority, immutable reviewer attribution, lifecycle-first conflict reconciliation, evidence-bound competing attribution, durable conflict events, atomic conflict snapshots, renderer query/subscription/reconnect/erasure, and typed user/graph review commands.
- [x] Consolidated the pending M5 schema change into one generated migration and hardened parent-table rebuilds to preserve mutation authorities, private payloads, and immutable diffs. Migration regression coverage proves legacy authority/payload survival.
- [x] Independent release audit found and resolved stale session subscriptions, missing proceed intake state, missing clarification routing, incomplete renderer review inputs, stale usage after erasure, non-atomic conflict snapshots, and inconsistent provider-failure queue reasons. No remaining P0-P2 finding was reported for authority, attribution/lifecycle, taxonomy/raw-origin, or erasure.
- [x] Focused integrated release verification passes 18 files / 202 tests; the real ProtocolServer/ArtisanClient file passes 21/21; the six migration/stale-contract regression suites pass 54/54; root formatting, lint, TypeScript, production frontend build, and diff checks pass.
- [x] `pnpm run validate` was run before the final milestone. Its first full test phase exposed and led to fixes for migration dependency loss and stale expectations; simultaneous Vitest runs in other Windows worktrees also pushed unrelated PowerShell/ACL tests beyond the unchanged 15-second ceiling. The final complete corpus rerun with a 60-second per-test diagnostic ceiling passed 109 files, 946 tests, and 2 intentional skips. M5-focused suites continue to pass under the ordinary 15-second ceiling; the shared host never provided an uncontended window for another default-timeout corpus run.
