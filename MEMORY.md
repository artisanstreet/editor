...
- [ ] Upstream clean-checkout runs `29656986818`, `29657787204`, and `29659051783` exposed Windows-only portability gaps after reaching the full validation gate. The first run found module-autoload, canonical temp-path, and deterministic retention-clock issues; the second narrowed failures to DACL bookkeeping and Git 8.3 aliases; the third proved ACL restoration green and showed that Effect/Node `realPath` preserves the runner's 8.3 spelling rather than providing file identity. Git root matching now uses filesystem device/inode identity with canonical-path fallback only where inode data is unavailable. M10 remains partial until the replacement upstream run passes.
