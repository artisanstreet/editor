...
- [x] Corrected the selector surface hierarchy after visual review: the entire popover uses `bg-background` rather than painting only the model ScrollArea black. Its triggers retain neutral hover/active styling and dropdown copy remains `text-base`/`text-sm`.
- [x] Derived the nested navbar geometry from the actual tokens: the popover is `rounded-2xl` (18px) with `p-2` (8px), so concentric inset corners require a 10px `rounded-lg` navbar (`R_outer = R_inner + padding`). It retains the prose-card gradient and `card` treatment, lifted to `from-foreground/10 to-foreground/5` for clearer separation.
- [x] The Popover content primitive uses the Barekey `card` utility without the shadcn ring or `shadow-2xl`; dropdowns inherit the shared card surface without selector-specific overrides.
- [x] Restyled the closed model selector from the current Barekey Docs command-picker control: `card-lg`, `bg-linear-to-b from-foreground/7.5 to-foreground/2.5`, muted-to-foreground interaction, padded content, and Tabler's two-way `Selector` glyph. Its radius is derived rather than copied: the 22px `rounded-3xl` panel minus its 4px `p-1` bevel yields the 18px `rounded-2xl` control radius.
- [x] Scaled down only the closed selector identity after visual review: the selected engine mark is `size-6`, the model heading is `text-base`, and the lab subtitle is `text-xs`; the control surface geometry remains unchanged.
- [x] Increased only the closed selector's left inset to `pl-4` while retaining `p-2` on the other sides, correcting the optical imbalance around the leading logo.
- [x] Returned the provider navigation to a compact horizontal strip. Scaled dropdown controls down to 32px provider tabs with 16px marks and model rows with 20px marks, `text-sm`/`text-xs` copy, `px-2.5 py-1.5`, and a 12rem viewport. Model hover now reuses the Docs sidebar's single translated `16% -> 7%` gradient highlight and shared fast/smooth motion tokens rather than painting each row independently.
- [x] Removed the model dropdown's unjustified 32rem prototype width; it now caps responsively at 20rem (320px), which fits the compact content and the containing side panel.
- [x] Fixed the typography boundary exposed by portalled UI: the application-level `--font-sans` token and its Tailwind reference now use Artisan Neo instead of PP Neue Montreal. Popovers and other body portals therefore inherit Artisan Neo without component-specific font overrides, while the existing Docs surface weight/tracking refinements remain scoped.
- [x] Added a persistent selected-provider indicator to the engine tabs rather than a competing hover state. One measured indicator uses `translate3d`, `--duration-fast` (250ms), and `--ease-smooth-out`; it snaps on first paint/resize, animates only between selections, layers below tab icons, and honors reduced motion.
- [x] Fixed the selected-provider indicator's runtime mount failure: `TabsList.ref` has a `null` bindable fallback, so the bound engine-surface state now initializes as `HTMLElement | null` with `null` rather than `undefined`, satisfying Svelte's bindable fallback contract.
- [x] Replaced the active provider's filled-card indicator with a shape-free top-edge light. Two overlapping blurred radial gradients originate just above the navbar and widen downward like a ceiling spotlight onto the selected logo; there is no caret, bar, triangle, border, or visible emitter. The light retains the measured 250ms smooth translation, first-paint/resize snap, and reduced-motion behavior.
- [x] Focused the selected-provider spotlight onto the 16px logo: its field narrowed from 56px to 32px, both radial ellipses tightened, and blur dropped from 2px to 1px so it no longer washes across neighboring space.
- [x] Shortened the spotlight layer to 24px so it illuminates only the logo's upper region rather than stretching through it. An elliptical alpha mask now diffuses the light to zero horizontally and vertically before the layer boundary, removing the perceptible rectangular cutoff.
- [x] Diffused the spotlight's top edge by moving both virtual gradient origins and the alpha-mask origin above the navbar, lowering peak opacity, and increasing local blur to 2px. The navbar now receives only the light's falloff rather than showing a painted bright fill at the source boundary.
- [x] Reverted the broad spotlight intensity increase after it made the cone extend visibly into the logo. Strength is now concentrated only in the compact source color: the 48% peak falls to 12% by 28% and fully transparent by 72%, while the broad cone and mask return to their softer accepted values.
- [x] Replaced the guessed geometric cone with a researched photometric-lobe approximation. Lighting references define beam width at 50% of peak intensity and field/spill around 10%, while directional LED distributions follow `I(θ) = I₀ cosᵐθ`; the UI light now uses a bowed elliptical mask centered below the source, with 50% and 10% alpha stops, plus a separate vertical brightness falloff. This produces rounded lamp-like sides rather than straight cone edges.
- [x] Softened the photometric lobe and accelerated its falloff: peak brightness dropped from 38% to 32%, the color reaches 10% by 26% and 2% by 52%, while the mask reaches its half-value at 42%, field value at 68%, and full transparency at 88%.
- [x] Stabilized the OpenCode provider mark across tab states. Its official vendored SVG intentionally uses `currentColor`, unlike the fixed-fill brand SVGs, so it inherited the tab primitive's muted-to-foreground interaction color; the SVG now owns `text-foreground` directly and no longer lightens on hover or selection.
