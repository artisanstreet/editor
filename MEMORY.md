...
- [ ] Upstream clean-checkout runs through `29660022493` exposed progressively narrower Windows-only portability gaps after reaching the full validation gate. Git root identity, ACL restoration, and libuv watcher stability are now green. The latest run's only failure proved that a temp root nested beneath the checkout invalidates non-repository fixtures because Git discovers the ancestor checkout. Hosted validation now exports an isolated run-scoped long path at the workspace drive root through `GITHUB_ENV`. M10 remains partial until the replacement upstream run passes.
