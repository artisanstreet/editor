...
- [x] The Popover content primitive no longer includes the shadcn ring, `shadow-2xl`, or a `card` utility; dropdowns now expose their plain semantic background without another card layer or selector-specific override.
...
- [x] Moved the model picker's provider navigation into a fixed-height vertical rail on the left of the dropdown, with the scrollable model table filling the right pane.
