...
- [x] Latest verified implementation checkpoint: `b70a7ec fix: fence external wake dispatch`
- [x] Local `HEAD`, its upstream, and `origin/codex/workspace-replace-approval` must be equal after every checkpoint push and before handoff; this was last verified on 2026-07-14 after external-wake erasure quiescence was pushed.
...
- [ ] Continue the `GitProvider` milestone with policy-controlled fetch, bounded failure drill-down, right-pane UI, and approval-bearing provider actions. Keep `GitLabProvider` explicitly deferred.
