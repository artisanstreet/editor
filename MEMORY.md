...
- [x] Latest verified implementation checkpoint: `9b14a40 feat: persist hosted review and ci state`
- [x] Local `HEAD`, its upstream, and `origin/codex/workspace-replace-approval` must be equal after every checkpoint push and before handoff; this was last verified on 2026-07-14 after the hosted pull-request read and durable review/CI projection checkpoints were pushed.
...
- [ ] Continue the `GitProvider` milestone with policy-controlled fetch, durable external waits, bounded failure drill-down, right-pane UI, and approval-bearing provider actions. Keep `GitLabProvider` explicitly deferred.
