...
- [ ] Upstream clean-checkout runs through `29659557191` exposed progressively narrower Windows-only portability gaps after reaching the full validation gate. The latest run proves ACL restoration and every Git production/service/integration path green with device/inode root identity; its sole failed assertion still compared the configured 8.3 spelling with Git's equivalent long spelling. Two libuv watcher workers also asserted because GitHub's short-spelled system temp root received long-spelled event paths. The assertion now checks file identity, and ordinary hosted validation uses a workspace-owned canonical temp root. M10 remains partial until the replacement upstream run passes.
...
- [x] Hosted ordinary validation now creates its temporary fixtures beneath the canonical checkout root and passes that path through `TEMP`/`TMP`, avoiding the GitHub runner's libuv short-name watcher assertion without changing product code or skipping watch coverage. The manual ordinary release gate uses the identical setup; protected self-hosted live/native jobs retain their host-managed temp policy.
