...
- [x] Replaced the active provider's filled-card indicator with a top-edge light: a small downward gradient caret acts as the emitter, a radial foreground cone illuminates the selected icon, and a restrained drop shadow sells the glow. The light retains the measured 250ms smooth translation between tabs, snaps on first paint/resize, and disables motion under reduced-motion.
