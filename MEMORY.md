...
- [ ] First upstream clean-checkout run `29656986818` reached the full validation gate but failed 11 files. Primary evidence is Windows-runner portability, not the new deep harness: `Microsoft.PowerShell.Security` could not autoload for ACL operations and cascaded through private config/model-behaviour tests; project-locator/workspace-registry expectations also disagreed on `RUNNER~1` versus canonical `runneradmin` temp paths. A separate workspace-evidence deterministic event-id collision is under focused review. Do not mark M10 complete until the root failures are fixed and a new clean-checkout run passes.
- [x] The clean-checkout Windows primary failures are fixed without changing runner images or weakening security: ACL capture/test inspection now use module-free exact .NET file ACL APIs instead of `Get-Acl`, and project-locator/workspace Git expectations canonicalize or compare the registry's authorized root across 8.3/long-path aliases. Focused verification passes 14 tests with one intended platform skip, TypeScript, full formatting, and diff checks. The remaining workspace-evidence restart idempotency failure is confirmed as a real production unique-key recovery defect and is being fixed at the repository boundary.
