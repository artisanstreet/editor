...
- [ ] Full product completion is active. Six isolated coordinator tasks own protocol ledger/rebuild, the built-in Artisan tool control plane, durable preview, orchestration/canonical surfaces, Marketplace Routine/MCP registries, and the deep harness/release gates. This task owns Electron plus the frontend and final integration. The Electron boundary is fixed: main brokers transferred MessagePorts, one utility process owns backend/PTY/Codex lifecycle, preload exposes only a narrow connection request/port-forwarding seam, and the renderer imports only the shell-neutral client. The renderer transport milestone is committed at `65e082b` plus review fixes at `431381c`; invalid/dropped offers close their ports, one deadline bounds invalid traffic, adapters acquire before Ready, and focused tests plus TypeScript pass. The shadcn shell correction is committed at `4246762`: Quick Open uses Command/Dialog, dirty close uses AlertDialog, responsive panes use Sheet, user actions use DropdownMenu, and the session pane uses Card/ScrollArea. The independently flagged bespoke Button implementation has been replaced with the registry version, exact `tailwind-variants` and Monaco dependencies are installed, and the serialized production frontend build passes. Electron commits `07f0eec` and `fc85261` provide the process split, custom protocol, shutdown/restart behavior, and a real package smoke, but are not accepted yet: independent review found missing packaged native runtime dependencies and an exception-safety leak during failed port handoff, so corrective work and another package smoke are active. A concurrent `pnpm exec`/dependency-add race temporarily pruned the root manifest and dependencies; the manifest, lockfile, and installation were restored before further work. Monaco remains under isolated implementation. UI work must keep using existing semantic tokens and shadcn primitives rather than inventing a parallel styling system. No completion status changes are allowed until the relevant public protocol/client, process-boundary, restart, accessibility, packaging, live-screen, and full validation gates pass.
