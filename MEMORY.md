...
- [x] Added a persistent selected-provider indicator to the engine tabs rather than a competing hover state. One measured gradient pill reuses the Docs sidebar highlight's `translate3d`, `--duration-fast` (250ms), and `--ease-smooth-out`; it snaps on first paint/resize, animates only between selections, layers below tab icons, and inherits the existing reduced-motion guard.
