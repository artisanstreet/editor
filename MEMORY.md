...
- [ ] First upstream clean-checkout run `29656986818` reached the full validation gate but failed 11 files. Primary evidence was Windows-runner portability: `Microsoft.PowerShell.Security` could not autoload for ACL operations and cascaded through private config/model-behaviour tests; project-locator/workspace-registry expectations also disagreed on `RUNNER~1` versus canonical `runneradmin` temp paths. The independent workspace-evidence restart failure was a backdated fixture whose live retention clock correctly erased the thread before retry. Root fixes are verified locally; do not mark M10 complete until the replacement clean-checkout run passes.
