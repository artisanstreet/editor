...
- [x] Latest verified implementation checkpoint: `b54f873 feat: expose hosted check failure detail`
- [x] Local `HEAD`, its upstream, and `origin/codex/workspace-replace-approval` must be equal after every checkpoint push and before handoff; this was last verified on 2026-07-14 after hosted check failure detail was pushed.
...
- [ ] Continue the `GitProvider` milestone with policy-controlled fetch, right-pane UI, and approval-bearing provider actions. Keep `GitLabProvider` explicitly deferred.
