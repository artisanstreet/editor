...
- [x] Applied the selector surface hierarchy: the provider navbar now uses the Barekey `card` utility over `bg-muted/40`, its tab triggers suppress hover/active decoration, the model ScrollArea uses `bg-background`, and dropdown model/provider copy uses normal `text-base`/`text-sm` sizing.
