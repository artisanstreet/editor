...
- [x] Latest verified implementation checkpoint: `9360425 docs: record backend git checkpoint`
...
- [ ] Full product completion is active. Six isolated coordinator tasks own protocol ledger/rebuild, the built-in Artisan tool control plane, durable preview, orchestration/canonical surfaces, Marketplace Routine/MCP registries, and the deep harness/release gates. This task owns Electron, frontend, and final integration. The accepted desktop boundary is main-brokered MessagePorts with backend/PTY/Codex lifecycle in one utility process, a narrow preload bridge, and renderer-only shell-neutral client imports. Commits `07f0eec`, `fc85261`, `f29ec5f`, and `c599af9` now cover hardened process ownership, bounded restart/shutdown, exception-safe port handoff, custom secure protocol, exact native-runtime staging, successful Windows package inspection/load smoke, parent-environment preservation for Codex CLI discovery, and a public startup-time `NODE_PATH` rather than private Node APIs. The renderer connection milestone is `65e082b` plus `431381c`. The shadcn-first shell is `4246762` plus registry/dependency restoration at `40ef81b`; serialized production builds pass and the main controls use Command/Dialog, AlertDialog, Sheet, DropdownMenu, Card, ScrollArea, and the registry Button. Monaco is implemented through `616745d`, `6519d2c`, and `89944a5` with workspace-qualified models, revision/conflict-safe saves, view-state restoration, markers, close/eviction, and exception-safe cleanup; production attachment awaits the coordinator file-discovery/read contract rather than inventing filesystem access. The live renderer is composed through `2c8dcf1`, `9cfe95f`, and `adfa508`: production fixture dependencies are removed, SubscriptionRef provides replaying state, thread selection/refresh races are fenced, authoritative removal clears stale work, ready/reconnect refreshes projections, lost subscriptions retry with bounded backoff and generation fencing, and the existing three-pane product shell keeps reachable Quick Open/tab/navigation/editor interaction seams using empty authoritative collections. Focused live tests, frontend lint/format, the production build, and fixture-dependency scan pass. A concurrent `pnpm exec`/dependency-add race temporarily pruned the root manifest and dependencies; the manifest, lockfile, and installation were restored before further work. UI work must keep using existing semantic tokens and shadcn primitives rather than inventing a parallel styling system. No completion status changes are allowed until coordinator integration, every live surface, accessibility/package gates, and full validation pass.

- [x] Root integration now includes preview through `a38c308`, final renderer-safe M5 surfaces through `a2cd693`, and the four M7 Marketplace checkpoints through `c0da9ab`. Additive conflict resolution preserves the M2 tool control plane, preview targets/assets, M5 transcript/session/surface/conflict routes, and complete Routine/MCP lifecycle APIs in one renderer-safe `ArtisanClient` and one Effect Layer graph. Post-integration fixes restored the fixed-clock restart runtime, retained `OutboundControlEnvelope` as a runtime Schema value, and split the 23-layer protocol composition at a named boundary to stay within Effect's typed pipe overloads. Root TypeScript passes and the focused M5/M7/restart batch passes 10 files and 118 tests. Release-harness integration, live frontend subscriptions/actions, and final package evidence remain active.

- [x] The M9/M10 code and workflow series is integrated through `44227c4`: deep public-surface architecture traversal, generated Engine state-machine scenarios, joined public-protocol workspace/rebuild integration, Windows clean-checkout workflows, stable authority identities, canonical temp roots, and fixture isolation are present. Root TypeScript and the focused deep/restart batch pass 5 files and 17 tests with 2 deliberate dependency skips after retaining both the fixed retention clock and a distinct restarted runtime identity. Independent integration review found one real remaining release gap: `.tests/deep/desktop/desktop-release-gates.test.ts` does not recognize `desktop-builder.yml`, and its string-only assertion is not package evidence. Final completion therefore still requires an actual package build/layout/native-load check plus forced packaged utility restart/reconnect/replay; M8-M10 must remain partial until that gate passes.
...
- [ ] M5 orchestration and canonical-surfaces coordination began on 2026-07-18 from upstream checkpoint `9360425`. The mandatory PRD/skill/repository audit confirmed four remaining owned slices: durable intake risk/assumption policy, complete steering routing/fallback coverage, usage plus canonical surface projection, and attributed workspace conflict/review handling. Two independent read-only audits found that usage is already normalized but raw-only, intake has no pre-run durable state, steering has no session setting or distinct provider-failure routing state, and controlled workspace changes are attributed but conflicts/reviews are not yet projected into orchestration surfaces. Implementation is in progress; no completion status has been advanced.
- [x] The first M5 intake checkpoint now has focused evidence: explicit provider-neutral risk assessments, pre-run questions with no engine outbox, recorded low-risk assumptions, exact/restart-safe clarification resolution, persisted project mentions/raw origin, thread-local auto-steer preference, strict answer authorization, durable table constraints, and thread-erasure cleanup. Independent review found six initial P1/P2 issues; the code fixes and nine-test orchestration-repository regression suite pass, and root `pnpm run check` is green. Final re-review and milestone commit are still pending, so the M5 matrix remains partial.
- [x] Final canonical-surface P2 fixes add SQLite-assigned cross-run projection ordering and validate every public item before persistence. Unsafe whitespace, control-character, or overlong provider provenance now becomes a schema-validated opaque native-action surface with provenance omitted, while the raw ledger retains forensic data. Unknown usage bases invalidate only the metrics they claim to report instead of being treated as deltas, and aggregate reads schema-validate both stored rows and the result with typed invariant failures for negative, fractional, or non-finite corruption. The 18-test focused surface suite, combined 36-test surface/retention/orchestration run, root TypeScript check, migration snapshot inspection, and diff check pass; the work remains uncommitted for coordinator integration.
...
- [x] M5 intake was independently re-reviewed, fixed for terminal-run clarification replacement, verified with ten focused repository tests plus root typecheck, committed at `cb8114e`, pushed to `origin/m5-orchestration-surfaces`, and proven equal upstream. The canonical surface taxonomy and usage-semantics foundation is also focused-green: strict Work/Time/Guidance/Routine/Capability/Process/Change/Permission/native-action schemas retain safe raw-origin references, Codex app-server totals are explicitly cumulative, exec usage is explicitly delta, and token counts reject negative/fractional/non-finite values. Durable surface/usage projections and renderer client wiring remain open.
...
- [ ] Cross-section validation dependency on 2026-07-19: full `pnpm run test` passed 115 files/977 tests with two skips but consistently failed the single M6-owned `workspace-evidence-recorder.test.ts` exact-retry-after-restart case (also fails alone) while inserting journal sequence 4. Marketplace tests are unaffected and pass 13 files/97 tests. M7 does not edit workspace-evidence ownership; its coordinator must reconcile that persistent journal idempotency regression before the repository-wide gate is fully green.
- [ ] M7 Effect capability research against pinned `effect@4.0.0-beta.97` found reusable Scope/acquire-release, Schema, Stream/Sink/Queue, Schedule/retry/timeout, Redacted/Config, experimental HTTP client, and generic experimental process APIs. Effect ships MCP server support but no outbound MCP client, OAuth/token lifecycle, OS secret vault, or Marketplace policy/ledger. The stdio MCP client remains a custom scoped Effect Service over Artisan's hardened process boundary because adding `@effect/platform-node` would not yet prove the repository's Windows job-object descendant-cleanup guarantees; HTTP, OAuth, secret storage, source installers, provider translators, and MCP client protocol remain narrow injectable boundaries with deterministic fakes.
...
- [x] Secure MCP transport infrastructure is committed and pushed at `24df8b1`. Default Layers remain inert and deny unavailable secret/OAuth/transport access. Explicit stdio sessions use the hardened Engine process factory, bounded concurrent NDJSON routing, initialization gates, schemas, pagination, crash/timeout/stderr cleanup, and scoped reconnect ownership. Explicit HTTP sessions use Effect's pinned client with manual redirects, origin/auth/session policy, bounded streamed decoding and pagination, OAuth/bearer secret references, health, and idempotent DELETE disconnect. Nineteen focused tests pass. Independent review/re-review closed notification, subprocess leak, initialization ordering, pending-bound, reconnect, compliant-schema, pagination, tool-error, redirect, scope-cleanup, and aggregation findings with no remaining P0-P2. Local and upstream were equal at `24df8b1d2ba4d0fb261a49e627e1a93e5d33361b` after push. Registry/service integration remains uncommitted and incomplete.
