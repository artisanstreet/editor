...
- [ ] Upstream clean-checkout runs `29656986818` and `29657787204` exposed Windows-only portability gaps after reaching the full validation gate. The first run found module-autoload, canonical temp-path, and deterministic retention-clock issues; the second narrowed the remaining failures to Windows adding the DACL `AI` bookkeeping flag during exact ACL restoration and Git inventory reporting an 8.3 alias for the registered long root. Both root fixes are now locally verified, but M10 remains partial until the replacement upstream run passes.
...
- [x] The second-run Windows regressions are fixed at their authority boundaries: ACL capture removes only the OS-added DACL auto-inherited control marker while preserving owner/group, protection/request flags, and every ACE; `WorkspaceGit.IsCurrentRoot` resolves inventory paths through the registered Effect filesystem authority and fails inaccessible paths closed as non-current. The production registry is covered with real junction/symlink, different-root, and missing-path cases. Independent re-review is clean through P1; TypeScript and 26 focused tests pass with one intentional skip.
