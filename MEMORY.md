...
- [x] Latest verified implementation checkpoint: `1191b81 fix: erase hosted clone thread state`
- [x] Local `HEAD`, its upstream, and `origin/codex/workspace-replace-approval` must be equal after every checkpoint push and before handoff; this was last verified on 2026-07-14 after the hosted-clone coordinator, renderer protocol, and erasure checkpoints were pushed.
...
- [ ] Continue the `GitProvider` milestone with policy-controlled fetch, stale-head-safe pull-request review and CI projections, durable external waits, bounded failure drill-down, and approval-bearing provider actions. Keep `GitLabProvider` explicitly deferred.
