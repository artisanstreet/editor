...
- [x] Added the private `@artisan/data` workspace module with 100 curated one-word Norwegian female agent names, 96 British female agent names including `Esmebeth`, and 70 independently curated Artisan thinking words including the exact semi-viral `Muhammading` reference spelling. The Norwegian names retain `Linnea` and `Elise`, use `Martha` instead of `Marta`, and keep a deliberate balance of naturalized everyday and distinctively Norwegian names. The PRD and an automatic catalog test now require shipped JSON datasets to stay at 100 entries or fewer; focused formatting, lint, catalog, JSON, uniqueness, NFC, and single-word checks pass. The current root TypeScript gate is blocked by unrelated in-progress workspace-approval edits in `.tests/protocol/workspace-changes-codec.test.ts`, so a new full-validation claim remains pending. Claude spinner references remain taste/provenance only because the published compilations are unlicensed or non-commercial and the PRD forbids blindly shipping them.
...
- [ ] Add public read/replace/list/review/rollback protocol commands and typed client methods.
