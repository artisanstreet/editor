...
- [x] Latest verified implementation checkpoint: `4e7ed41 feat: expose hosted git mutations to renderer`
- [x] Local `HEAD`, its upstream, and `origin/codex/workspace-replace-approval` must be equal after every checkpoint push and before handoff; this was last verified on 2026-07-16 at `4e7ed41 feat: expose hosted git mutations to renderer`.
...
- [x] Reconfirmed with the user on 2026-07-15: no `.github` workflow or other remote-testing detour remains. The deleted native-verification workflow was a one-off diagnostic, all routine verification runs locally, and the canonical native gate passed locally again on 2026-07-15. The earlier BSOD remains classified as a one-off storage-driver incident rather than a reason to move testing off-machine.
...
- [x] Canonical engine capability activity and run-scoped raw-observation identity are committed and pushed at `d30254c`. Tool lifecycle and opaque native actions now publish bounded source-safe events with stable SHA-256 aliases derived from persisted run ownership; provider names, IDs, arguments, results, actions, and diagnostics remain private raw evidence. The shared ordinary/graph raw ledger now keys observations by `(run_id, observation_id)`, with a generated migration that preserves legacy rows and prevents one run from suppressing another. Six focused suites pass 20 tests; TypeScript, scoped Oxlint/Oxfmt, schema generation, and independent review are clean. Local `HEAD`, upstream, and the remote branch were verified equal at `d30254c8dee40ee5be1403b01ee6949a95ea56b8`. The durable Artisan tool control plane is the active slice.
...
- [x] Typed preview-target protocol, durable SQLite core, ProtocolServer/runtime composition, typed `ArtisanClient` integration, and the production local health probe are committed and pushed at `fe4edf5`, `ac4b5d5`, `ad14f53`, and `f689a56`. The Node probe makes one scope-owned `HEAD` request without redirect following, validates and pins localhost DNS answers to loopback, preserves the original Host/TLS identity, reports bounded integer latency, and maps transport failures to source-safe health. Explicit probe Layers override the production default. Five neighboring preview suites pass 69 tests; TypeScript, scoped Oxlint/formatting, and independent review pass. The review-requested DNS-pinning regression test is included. The external-browser inspection lifecycle remains active; no remote runner or workflow is used.
- [x] Rich-link metadata and binary favicon transport are committed and pushed at `bb5520d` and `a8302f5`. Strict provider-neutral request/result codecs expose bounded page names, canonical URLs, cache provenance, and content-addressed asset references without raw bytes. The production resolver applies credential, redirect, public-address, DNS-pinning, response-size, media-signature, canonical-URL, and remote-control-character policy; ProtocolServer strict-validates complete results before enqueueing and emits source-safe correlated errors. One memoized Effect Layer shares the bounded in-memory asset store between metadata resolution and `BackendBinaryStreamSourceLive`. The typed `ArtisanClient` retries the exact pending envelope after a dropped result, and the real MessageChannel harness proves exact bytes, namespace isolation, LRU eviction, restart loss, malformed-service containment, and continued connection health. Eight neighboring suites pass 69 tests; the complete local one-worker gate passes 167 files with one intentional file skip, 1,403 tests, and 4 intentional test skips. TypeScript, Oxlint, scoped formatting, diff checks, and the production frontend build pass. Independent critical review and remedy re-review report no P0-P2 finding; no remote runner or workflow was used.
- [x] External-browser lifecycle and generation-specific target-removal fencing are committed and pushed at `06882cf`, `430037a`, and `fea2e45`. The Effect lifecycle opens the system browser, owns scoped inspection connectors, persists launch/attach/detach/disconnect state, recovers interrupted work, settles pending removal fences on startup and quiescence, and protects replacement generations and thread erasure. Hard revocation fences existing and late connector authority across timeout, failure, explicit detach, observed disconnect, target removal, thread erasure, expired recovery, and runtime disposal; current fences require exact generation-bound evidence while legacy generationless removal events remain replayable. The focused 12-file matrix passes 107 tests, the seven-file remedy matrix passes 74 tests, and eight critical race tests pass five consecutive repetitions for 40/40 runs. The complete local one-worker gate passes 172 files with one intentional file skip, 1,464 tests, and 4 intentional test skips. TypeScript, Oxlint, scoped formatting, diff checks, migration generation, and the production frontend build pass locally. Independent critical review and remedy re-review report no P0-P2 finding. Aggregate formatting remains blocked only by the unrelated user-owned `backend-completion-matrix.md` hunk; no remote runner or workflow was used.
- [x] Hosted-provider mutation contracts and GitHub V1 execution are committed and pushed at `a1b8851` and `372aae7`. The shared `ProcessRunner` can now explicitly own complete subprocess trees through the existing Engine process factory, preserving bounded output and closing descendants on interruption. Canonical provider-neutral requests cover review replies and resolution, reviewer requests, workflow rerun/cancel, and merge strategy; GitHub binds every action to the selected account, exact repository/PR/head/native origins, validates exact provider results, and classifies any post-launch ambiguity as non-retryable `outcome_unknown`. The focused four-file matrix passes 73 tests; the complete local one-worker gate passes 172 files with one intentional file skip, 1,477 tests, and 4 intentional test skips. TypeScript, Oxlint, scoped formatting, diff checks, and the production frontend build pass locally. Independent implementation and remedy reviews report no P0-P2 finding; no remote runner or workflow was used. Durable approval, claims, recovery, erasure, and protocol integration are the active slice.
- [x] Hosted-provider mutation approval contracts are committed and pushed at `3dfc7dc`. The renderer command binds the exact canonical mutation to an explicit provider host/account selection; the public approval lifecycle preserves that source-safe selection and target summary while canonical rejection/unknown reasons cannot retain provider output. SQLite separates public approval state from private exact request/provider-result artifacts and durable workspace claims, with constraints for pre-launch recovery, post-launch ambiguity, private scrubbing, and terminal outcomes. The focused protocol and migration-backed snapshot suites pass 11 tests; TypeScript, Oxlint, scoped formatting, diff checks, and Drizzle schema-drift generation pass locally. Request/decision persistence is the active sub-slice, followed by claims/execution recovery, coordinator dispatch, erasure, and MessagePort integration.
- [x] Hosted-provider mutation request and decision persistence is committed and pushed at `b364ca8`, `7a8b6df`, and `14627c8`. The global event codec now recognizes hosted mutation approvals; the Effect `Crypto`-backed repository journals source-safe commands/events separately from exact private operations, binds admission to the explicit provider account, durable project attachment, one visible ready checkout, current snapshot, open pull request, exact branch/head/native targets, and rejects first decisions after thread erasure starts. Exact replay survives restart and denied-request scrubbing without resurrecting reply text; malformed public/private/event state fails closed, and two SQLite runtimes converge exact requests while rejecting competing intent. Five neighboring suites pass 30 tests; TypeScript, scoped Oxlint/formatting, Drizzle schema-drift generation, diff checks, and independent remedy re-review pass locally. Durable execution claims, provider-result recovery, terminal settlement, full thread erasure, coordinator dispatch, and MessagePort integration remain active.
- [x] Hosted-provider execution claims and thread-erasure ownership are committed and pushed at `2ce2b92`, `82239ae`, `b97b7a3`, and `b438786`. Durable claim/lease/start/completion evidence is strict-validated at dispatch, read, renewal, and irreversible launch boundaries; checkout, local mutation, and hosted mutation claims are mutually exclusive across two SQLite runtimes; requested, approved, and executing hosted work fences retention before and after quiescence; terminal state is deleted in foreign-key-safe order; and corrupt cross-thread claims fail closed without releasing workspace ownership. The seven-suite matrix passes 63 tests, both public two-runtime races passed 10 consecutive focused repetitions, and the complete one-worker gate passes 175 files with one intentional file skip, 1,505 tests, and 4 intentional skips. TypeScript, full lint, scoped formatting, diff checks, and the production frontend build pass; aggregate validation remains blocked only by the unrelated user-owned formatting in `backend-completion-matrix.md`. Independent review and remedy re-review report no P0-P2 finding.
- [x] Hosted-provider reply-result binding and durable outcome recovery are committed and pushed at `cfbd0a0` and `caf41a5`. Canonical reply results now bind a new review comment to its exact parent thread and provider; SQLite durably records normalized provider evidence before settlement, classifies owned/waiting/recoverable/quarantine work, uses cross-runtime CAS recovery, serializes settlement and quarantine behind the live workspace gate, exact-replays terminal outcomes, and scrubs private operations, selections, and recorded results. The nine-suite provider/claim/erasure matrix passes 100 tests; TypeScript, scoped Oxlint, Oxfmt, and diff checks pass. Independent review found and the remedy re-review closed the parent-thread binding P1, with no remaining P0-P2 finding. Coordinator dispatch and MessagePort integration remain active.
- [x] Hosted-provider mutation coordination and production composition are committed and pushed at `26144b9` and `a465416`. The scoped Effect coordinator serializes durable approval dispatch, executes the selected provider behind the shared workspace gate, records normalized provider evidence before settlement, never replays an irreversible launch after uncertain persistence, and recovers or quarantines restart state by durable launch evidence. The production runtime now owns and exposes the repository/coordinator pair, and thread erasure quiesces hosted mutation dispatch with every other thread-owned resource. The focused coordinator, repository, and production-composition matrix passes 45 tests; TypeScript, scoped Oxlint, and diff checks pass. The previously reviewed production-composition and quiescer gaps are closed; protocol and typed MessagePort integration remain active.
- [x] Hosted-provider mutation protocol and typed renderer integration are committed and pushed at `9b0a355` and `4e7ed41`. Strict request/query/decision/result envelopes route through the production ProtocolServer with source-safe errors and correlated approval projections; the typed `ArtisanClient` and fixture expose the same durable surface. Real SQLite protocol coverage proves request, approval, terminal application, query, and changed-intent rejection. The real Git/SQLite/MessagePort harness proves requested-state restoration after backend restart, exact decision replay after a deliberately dropped receipt and renderer reconnect, a `duplicate` retry receipt, redacted public projections, and exactly one provider execution. The focused six-file matrix passes 54 tests; the complete one-worker gate passes 178 files with one intentional file skip, 1,537 tests, and 4 intentional skips. TypeScript, root Oxlint, scoped Oxfmt, diff checks, and the production frontend build pass locally. Aggregate `pnpm run validate` remains blocked only by the unrelated user-owned formatting in `backend-completion-matrix.md`. Independent review reports no P0-P2 finding; right-pane UI remains and `GitLabProvider` stays deferred.
...
- [x] Add the production local preview health probe.
- [x] Add binary asset routes.
- [x] Add the external-browser inspection lifecycle.
