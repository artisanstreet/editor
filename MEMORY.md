...
- [x] Upstream clean-checkout run `29660258073` passed on commit `a3a16cd`: checkout, pinned pnpm/Node setup, frozen install, canonical isolated temp preparation, formatting, lint, TypeScript, production static renderer build, and all 108 Vitest files completed successfully. This proves the ordinary hosted gate and the Windows ACL/Git/watcher portability fixes. M10 remains partial only for its explicit Electron packaging/native-unpack and dependency-gated product surfaces, not for ordinary CI execution.
- [x] The clean-checkout primary failures are fixed without changing runner images or weakening security: ACL capture/test inspection now use module-free exact .NET file ACL APIs instead of `Get-Acl`, project-locator/workspace Git expectations tolerate canonical 8.3/long-path aliases, and every full recorder runtime uses a fixed retention clock matching deterministic evidence timestamps. Restart coverage now proves both exact duplicate recovery and changed-intent conflict. Focused verification passes 21 protocol/composition tests, 7 recorder tests, 14 Windows/path tests with one intended platform skip, TypeScript, formatting, and diff checks.
- [x] The later Windows regressions are fixed at their authority boundaries: ACL capture removes only the OS-added DACL auto-inherited control marker while preserving owner/group, protection/request flags, and every ACE; `WorkspaceGit.IsCurrentRoot` compares Effect filesystem device/inode identity and fails inaccessible paths closed as non-current. Production registry coverage uses real junction/symlink, different-root, and missing-path cases. TypeScript and the Git/deep focused suites pass; final independent review and replacement upstream execution remain required.
- [x] Hosted ordinary validation creates temporary fixtures in an isolated run-scoped long path at the workspace drive root and exports it through `TEMP`/`TMP`, avoiding both the GitHub runner's libuv short-name watcher assertion and ancestor-repository discovery without changing product code or skipping coverage. The manual ordinary release gate uses the identical setup; protected self-hosted live/native jobs retain their host-managed temp policy.
- [x] The M9/M10 checkpoint at `a3a16cd` was pushed to `origin/m9-deep-harness-release`; local `HEAD` and upstream were equal at `a3a16cd6823aaf5df6bc52014a0602cee8371cd1`, and GitHub Validate run `29660258073` completed successfully before this evidence update.
- [x] Final M9/M10 local milestone verification on 2026-07-18: `pnpm run validate` passes formatting, lint, TypeScript, the strict production static frontend build, and 108 Vitest files with 896 passing tests plus 4 intentional opt-in/dependency skips. An earlier shell-level timeout left one verified Vitest parent/worker pair orphaned in this worktree; only those two processes were terminated, after which the standalone full suite and canonical validation both passed without changing Vitest's 15-second per-test timeout.
- [x] M9/M10 desktop integration gates are additive and product-slice-safe: the deep desktop test enforces the existing strict static renderer output and renderer-safe typed Electron MessagePort shapes; the ordinary release gate runs the production frontend build and all renderer/transport tests. The release policy maps current in-process MessageChannel reconnect/replay evidence and explicitly withholds packaged restart equivalence. Electron package-layout/native-unpack and mounted browser accessibility/responsive tests are named conditional dependency gates until the Electron bootstrap and browser runner land; no Electron/UI product implementation was duplicated.
- [x] M9 joined public-runtime scenario now uses one temporary workspace, committed Git repository, SQLite database, deterministic runtime metadata, scoped fake Engine, fake bounded native store, and public `ProtocolServer`. It publicly creates a thread and starts the fake Engine, waits for durable running authority, accepts one attributed replacement, returns an exact duplicate receipt without a second filesystem effect, verifies the visible Git diff, restarts the backend, rebuilds public change/diff projections, and proves Engine/store/directory cleanup. Focused verification passes. The real fake child-process backend, generated provider restart/orphan recovery, and packaged Electron path remain explicit dependency gates.
- [x] M10 clean-checkout and release policy is implemented on `m9-deep-harness-release`: ordinary push/PR/manual CI uses Windows 2025, Node 24, pinned pnpm 11.7.0, a frozen lockfile, and the full repository validation gate. Manual release validation always repeats that ordinary gate; live Codex account-read compatibility and native addon smoke/recovery are independent default-off inputs restricted to dedicated self-hosted runner labels and protected environments. No workflow uploads fixtures or artifacts, and `docs/release/validation-policy.md` records sanitization and the Codex-only production-engine boundary. Workflow formatting and semantic policy assertions pass; `actionlint` is not locally installed, so GitHub execution remains the clean-checkout proof gate after push.
- [x] M9 generated Engine state-machine coverage added under `.tests/deep/generated`: eight fixed reproducible seeds prove exact command-id retry effects, changed-intent conflict rejection, command/observation correlation, contiguous observation sequences, single terminal observation, exact-once scoped cleanup, and concurrent emit/finish interleavings. Focused verification passes 2 generated tests together with the 5 architecture tests, and `pnpm run check` passes. External malformed-frame injection, public reconnect/replay, multi-agent orchestration, and provider restart/orphan recovery remain dependency gates rather than overclaimed coverage.
...
- [x] Added deterministic public-runtime restart rebuild and equivalence coverage for the merged thread/workspace/Git surfaces; provider-process orphan recovery remains dependency-gated.
- [x] Added seeded generated state-machine scenarios for retries, changed-intent conflicts, correlation, continuity, terminality, cleanup, and concurrent emit/finish interleavings; external reconnect/malformed-frame and multi-agent domains remain dependency-gated.
- [x] Added one deep public-protocol temporary workspace/Git/SQLite/fake-engine integration scenario.
- [x] Added complete architecture-boundary and surface-normalization gates for all currently merged public surfaces, with unavailable M7/M8/Electron domains explicitly blocked.
- [x] Added clean-checkout and manual release workflows; live CLI/native probes are explicit protected self-hosted opt-ins and never run silently in ordinary CI. Upstream green execution remains required before M10 can advance beyond partial.
