...
describe("usage interruption card", () => {
	...
	it("uses a scoped absolute-time countdown and revisioned commands", () => {
		...
		expect(route).toContain("ScheduleConversationAndInteractionReconciliation");
		expect(route).toContain('{ concurrency: "unbounded", discard: true }');
		expect(route).not.toContain("yield* Resync.pipe(Effect.ignore)");
	});
	...
	it("persists the global default through the session-defaults controller", () => {
		...
		const settings = Read(
			"modules/frontend/src/routes/components/settings/usage-recovery.svelte",
		);
